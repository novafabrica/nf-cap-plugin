...
require 'capistrano/novafabrica/databases'
...
configuration.load do
  ...
  _cset :user,          'deploy'
  # Application details
  _cset(:short_name)      { abort "Please specify the short name of your application, set :app_name, 'foo'" }
  _cset(:runner)        { user }
  _cset :use_sudo,      false

  # SCM settings
  _cset(:appdir)        { "/var/wwww/#{short_name}_#{rails_env}" }
  _cset :scm,           'git'
